<?php
// controllers/supervendedor/RegistroRecibirDController.php
...
require_once '../../models/ModeloDevolucion.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($remision_id) || empty($conductor) || empty($placa) || empty($transporte_id)) {
        ...
        if (!in_array($tipo, ['propio', 'duratranz'])) {
            ...
            // Obtener datos de cliente, vendedor, rol y departamento asociados a la remisión
            ...
            if (!$clienteYVendedor) {
                ...
                $rol_id = $clienteYVendedor['rol_id'];
                $remision_departamento_id = $clienteYVendedor['remision_departamento_id'];
                $is_supervendedor = ($rol_id == 4);
                ...
                $stmtDept->execute(['usuario_id' => $_SESSION['usuario_id']]);
                $usuario_departamento_id = $stmtDept->fetchColumn();
                ...
                if (empty($error)) {
                    try {
                        ...
                        // Insertar la recepción incluyendo el tipo y ajustar pallets si es necesario
                        insertarRecibir(
                            $pdo, 
                            $remision_id, 
                            $conductor, 
                            ...
                            $vendedor_id, // Usar el vendedor de la remisión en lugar del usuario logueado
                            $transporte_id, 
