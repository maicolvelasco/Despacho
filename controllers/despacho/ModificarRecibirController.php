...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (!isset($_POST['id']) || !isset($_POST['tipo']) || !isset($_POST['pallets'])) {
        ...
        header("Location: ModificarRecibir.php?id={$_POST['id']}&tipo={$_POST['tipo']}");
        exit();
    }
    ...
    if ($tipo !== 'envio' && $tipo !== 'recibir') {
        ...
        header("Location: ModificarRecibir.php?id={$id}&tipo={$tipo}");
        exit();
    }
    ...
    foreach ($pallets as $pallet) {
        if (!isset($pallet['cantidad']) || !is_numeric($pallet['cantidad']) || intval($pallet['cantidad']) < 0) {
            ...
            header("Location: Modificar_RecibirR.php?id={$id}&tipo={$tipo}");
            exit();
        }
    }
    ...
    if ($resultado) {
        ...
        header("Location: ../../views/despacho/Detalles.php?id={$id}&tipo={$tipo}");
        exit();
    } else {
