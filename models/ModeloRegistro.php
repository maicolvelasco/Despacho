...
function getUserRole($pdo, $usuario_id) {
    $stmt = $pdo->prepare("SELECT rol_id FROM usuarios WHERE id = :usuario_id");
...
function insertarEnvio($pdo, $remision_id, $conductor, $placa, $cliente_id, $usuario_id, $transporte_id, $titular_id, $tipo, $imagenes, $descripcion_imagen, $pallets) {
    ...
    // Insertar imágenes capturadas si existen
    if (!empty($imagenes) && is_array($imagenes)) {
        foreach ($imagenes as $index => $imagen_base64) {
            ...
            // Comprimir la imagen si es demasiado grande
...
function getTitularByCode($pdo, $code) {
    $stmt = $pdo->prepare("SELECT t.id, t.nombre, t.codigo, t.usuario_id, u.nombre as vendedor_nombre, u.id as vendedor_id FROM titular t JOIN usuarios u ON t.usuario_id = u.id WHERE t.codigo = :code");
    $stmt->execute(['code' => $code]);
    return $stmt->fetch(PDO::FETCH_ASSOC);
}

function getClienteByCode($pdo, $code) {
    $stmt = $pdo->prepare("SELECT c.id, c.nombre, c.codigo, c.titular_id, t.nombre as titular_nombre, u.nombre as vendedor_nombre, u.id as vendedor_id FROM clientes c LEFT JOIN titular t ON c.titular_id = t.id JOIN usuarios u ON t.usuario_id = u.id WHERE c.codigo = :code");
    $stmt->execute(['code' => $code]);
    return $stmt->fetch(PDO::FETCH_ASSOC);
}
?>