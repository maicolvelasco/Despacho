...
try {
    ...
    $department_id = $supervendedorModel->getDepartmentIdByUserId($_SESSION['usuario_id']);
    
    if (!$department_id) {
        die('No se pudo determinar el departamento del usuario.');
    }
    ...
    // Crear nuevo PDF
    $pdf = new PDF('L'); // Landscape para más espacio
    $pdf->AddPage(); 
    ...
    foreach ($totalesPorRemision as $remision => $datos) {
        ...
        if ($cantidadPendiente > 0) {
            ...
            $fillColor = ($fechaFin <= $today) ? [255, 230, 230] : [255, 255, 255]; 
    ...
    // Salida del PDF
    $pdf->Output('D', 'remisiones_faltantes_departamento_' . date('Y-m-d') . '.pdf');
} catch (Exception $e) {
